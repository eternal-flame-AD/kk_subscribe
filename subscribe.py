...
def send_comic(title, issue):
    uri = issue[1]
    response = bs4.BeautifulSoup(download_data("www.kuaikanmanhua.com",uri),"html5lib")
    ...
    for i in range(len(imglist)):
        ...
        with open("img/{}.jpg".format(i), mode="wb") as f:
            print(src)
            f.write(download_data(urllib.parse.urlparse(src).netloc,src, https=urllib.parse.urlparse(src).scheme=="https"))
        imgs.append("img/{}.jpg".format(i))
    concat_image(imgs)
    email = emailhandler.EmailToSend(TITLE_FORMAT_STRING.format(title=title, issue=issue[0]), MAIL_SENDER, ','.join(SUBSCRIBER))
